...
const AllPrizes = () => {
  ...
  const deletePrize = async () => {
    ...
    try {
      ...
      toast.success("Prize deleted successfully!", { id: toastId });
    } catch (error) {
      toast.error("Failed to delete prize", { id: toastId });
    }
  };

  const updatePaymentStatus = async (prizeId, status) => {
    let toastId = toast.loading("Updating payment status...");
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to update payment status", { id: toastId });
    }
  };
      ...
      <div className="text-white">
        <div className="flex flex-wrap justify-end gap-4 mb-4">
          <input
            ...
            placeholder="Search by winner, tournament, or position"
            ...
            <table className="w-full table-auto text-left text-white border-separate border-spacing-y-2 border border-transparent">
              <thead className="bg-(--secondary) p-2 rounded-sm text-white">
                <tr className="rounded-2xl">
                  ...
                  <th className="p-3 font-medium whitespace-nowrap w-max">Winner</th>
                  <th className="p-3 font-medium whitespace-nowrap w-max">Position</th>
                  <th className="p-3 font-medium whitespace-nowrap w-max">Prize Value</th>
              ...
              <div className="fixed inset-0 flex items-center justify-center bg-black/50">
                <div className="bg-(--primary) p-6 rounded-lg shadow-lg w-1/3">
                  ...
                  <div className="mt-4 flex justify-end gap-3">
                    ...
                    <button className="px-4 py-2 bg-red-600 text-white rounded" onClick={deletePrize}>Delete</button>
                  </div>
                </div>
              </div>
            )}
